...
class QuizContentDialog extends StatefulWidget {
  const QuizContentDialog({super.key, required this.inQuiz});

  final QuizModel inQuiz;

  @override
  State<QuizContentDialog> createState() => _QuizContentDialogState();
}

class _QuizContentDialogState extends State<QuizContentDialog> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      APIResult resQuiz = await examAPI.readQuizOne(loginSession!.token, widget.inQuiz.id);
      quiz = resQuiz.result[0] as QuizModel;

      final ContentAPI contentAPI = ContentAPI();
      APIResult resContent = await contentAPI.readOne(loginSession!.token, quiz!.contentid);
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                const Text(' เนื้อหาข้อสอบ', style: TextStyle(fontSize: 30,),),
                              ],
                            ),
                            IconButton(
  ...
  contentSubmit() async {
    ...
    APIResult resDelete = await contentAPI.deleteOne(loginSession!.token, quiz!.contentid);

    Uint8List bytes = Uint8List.fromList(utf8.encode(jsonEncode(_controller.document.toDelta().toJson())));
    PlatformFile content = PlatformFile(name: 'q${quiz!.id}-${DateFormat('yyyyMMddHHmmss').format(DateTime.now().subtract(DateTime.now().timeZoneOffset))}.txt', size: bytes.lengthInBytes, bytes: bytes);
    ...
    if(resCreate.status == 1) {
      ...
      for(int i = 0; i < length; i++) {
        choices.add(
          CreateQuizChoiceModel(
            ...
            feedbackid: quiz!.choices![i].feedbackid
          )
        );
      }
      ...
      final ExamAPI examAPI = ExamAPI();
      // ignore: unused_local_variable
      APIResult resUpdateItem = await examAPI.updateQuizOne(loginSession!.token, quiz!.id, quiz!.exam_id, quiz!.quizno, quiz!.quizminute, quiz!.question, contentid, quiz!.mediaid, choices);
      ...
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(snackbar.showSuccess('บันทึกเนื้อหาข้อสอบเรียบร้อยแล้ว', globalConstraints, colorScheme));
    } else {
      // ignore: use_build_context_synchronously
      context.pop();
